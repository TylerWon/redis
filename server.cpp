...
struct Conn {
    ...
    std::vector<char> incoming;  // data to be parsed by the application
    std::vector<char> outgoing;  // responses generated by the application
};
...
bool recv_data(Conn *conn) {
    char buf[64 * 1024]; // 64 MB, large size is to handle pipelined requests
    ...
    conn->incoming.insert(conn->incoming.end(), buf, buf + (uint32_t) recvd);
...
Request *parse_request(Conn *conn) {
    ...
    conn->incoming.erase(conn->incoming.begin(), conn->incoming.begin() + Request::REQ_LEN_HEADER_SIZE + request->length());
...
void handle_send(Conn *conn) {
    ...
    conn->outgoing.erase(conn->outgoing.begin(), conn->outgoing.begin() + sent);
...
void handle_recv(Conn *conn) {
    ...
    while (Request *request = parse_request(conn)) {
        ...
        conn->outgoing.insert(conn->outgoing.end(), buf, buf + buf_len);
