...
const time_t IDLE_TIMEOUT_MS = 5 * 1000;
...
/**
 * Idle timer for a connection. 
 * 
 * When expiry_time_ms is reached, the connection has been idle for too long and should be removed. 
 */
struct IdleTimer {
...
void handle_new_connection(int listener) {
    ...
    conn->fd = client;
    conn->want_read = true;
    conn->idle_timer.expiry_time_ms = get_time_ms() + IDLE_TIMEOUT_MS;
...
/**
 * Resets the expiry time of an IdleTimer.
 * 
 * Since its expiry time is reset, the timer is also moved to the end of the idle timer queue because it will expire
 * the latest.
 * 
 * @param timer The timer to reset.
 */
void reset_idle_timer(IdleTimer *timer) {
    timer->expiry_time_ms = get_time_ms() + IDLE_TIMEOUT_MS;
