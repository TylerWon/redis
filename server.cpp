...
struct Conn {
    ...
    // timers
    Timer idle_timer; // if expiry time reached, connection has been idle for too long
};
...
int32_t get_time_until_timeout() {
    ...
    Timer *timer = container_of(node, Timer, node);
 ...
 */
void reset_idle_timer(Timer *timer) {
    timer->reset();
